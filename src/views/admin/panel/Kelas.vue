      ...
      <div class="container-fluid text-light" style="max-width: 68.5rem;">
        <div class="d-md-flex d-grid justify-content-between">
          <div class="mt-4 mx-2 d-flex gap-2">
            <label class="fs-5" style="font-weight: 500; color: rgba(255, 255, 255, 1);"><span><i class="bi-building-fill"></i></span> Kelas</label>
            <label class=" fw-lighter mt-2" style="font-size: 0.9rem; color: rgba(255, 255, 255, 1);">Mengelola kelas sesuai topik pembelajaran anda</label>
          </div>
          <div class="mt-4 mx-2 d-flex gap-2">
            ...
            <label class="text-muted" style="font-size: 0.8rem;">Kelas</label>
          </div>
        </div>
          ...
          <div class="card shadow-lg" style="border: none; background-color: rgba(255, 255, 255, 0.05); height: 30rem;">
            <div class="card-header d-grid" style="">
              <div class="d-flex justify-content-between">
                <div class="d-grid">
                  <label class="fs-6 text-start fw-normal" style="font-weight: 500; color: rgba(255, 255, 255, 1);">Kelas Anda</label>
                  <p class="fs-6 fw-lighter text-start" style="color: rgba(255, 255, 255, 1);">Lihat kelas yang telah didaftarkan</p>
                </div>
                <div class="d-flex mt-0 py-3">
                  <button @click="addKelas" class="text-decoration-none" style="color: rgba(255, 255, 255, 0.5)"><span><i class="bi-plus-circle"></i></span> Buat Kelas</button>
                </div>
              </div>
            </div>
            <div class="card-body" style="max-height: 20rem; overflow-y: auto;">
              <table class="table table-bordered border-secondary">
                ...
                <tr>
                  ...
                  <td class="text-start">Jumlah Siswa</td>
                  <td class="text-start">Total Siswa (max)</td>
                  <td class="text-start">Dibuat Pada</td>
                  <td class="text-start">Terakhir di Edit</td>
                ...
                <tr v-for="(kelas, index) in data_kelas" :key="data_kelas.id">
                  <td class="text-start">{{ index + 1 }}</td>
                  <td class="text-start text-secondary">{{ kelas.id }}</td>
                  <td class="text-start">{{ kelas.name }}</td>
                  <td class="text-start">{{ kelas.total_students }}</td>
                  <td class="text-start">{{ kelas.total_max_students }}</td>
                  <td class="text-start">{{ kelas.createdAt }}</td>
                  <td class="text-start">{{ kelas.updatedAt }}</td>
                  <td class="text-start d-flex justify-content-start gap-2">
                    <button type="button" @click="editKelas(kelas.id, kelas.name, kelas.total_max_students)" class="btn btn-sm btn-primary"><span class="text-light"><i class="bi-pen-fill"></i></span> Edit</button>
                    <button type="button" @click="deleteKelas(kelas.id)" class="btn btn-sm btn-danger"><span class="text-light"><i class="bi-trash-fill"></i> Hapus</span></button>
                    <button type="button" @click="copyId(kelas.id, kelas.name)" class="btn btn-sm btn-success"><span class="text-light"><i class="bi-clipboard-fill"></i> Copy ID</span></button>
                  </td>
                </tr>
                </tbody>
              </table>
...
export default {
  ...
  methods: {
    ...
    editKelas(id, classname, maxsiswa) {
      this.$router.push(`kelas/edit/${id}/${classname}/${maxsiswa}`);
    },

    async copyId(kelas_id, classname) {
      try {
        const textarea = document.createElement("textarea");
        textarea.value = kelas_id;
        document.body.appendChild(textarea);
        textarea.select();
        document.execCommand("copy");
        document.body.removeChild(textarea);
        alert(`Copied ID: ${kelas_id} from ${classname}`);
      } catch (err) {
    ...
    async deleteKelas(kelas_id) {
      try {
        ...
        if (confirmation) {
          const response = await axios.post('http://localhost:3000/api/kelas/delete', {
            id: kelas_id,
          });
