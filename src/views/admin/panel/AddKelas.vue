...
export default defineComponent({
  ...
  methods: {
    ...
    async addClass() {
      try {
        const response = await axios.post(`http://localhost:3000/api/kelas/create`, {
          id: this.generateRandomId(),
          name: this.addKelas.name,
          maxsiswa: this.addKelas.maxsiswa
        });
        ...
        <div id="daftar" class="py-4 mx-2" style="margin-bottom: 20rem;">
          <div class="card shadow-lg" style="border: none; background-color: rgba(255, 255, 255, 0.05); height: 20rem;">
            <div class="card-header d-grid py-3" style="">
              <label class="fs-6 text-start fw-bold" style="font-weight: 500; color: rgba(255, 255, 255, 1);">Buat Kelas</label>
            </div>
            <div class="card-body" style="max-height: 20rem; overflow-y: auto;">
              <table class="table table-bordered border-secondary">
                ...
                <tr>
                  <td class="text-start">
                    <input v-model="addKelas.name" class="bg-transparent border-1 form-control form-control-sm text-light">
                  </td>
                  <td class="text-start">
                    <input v-model="addKelas.maxsiswa" class="bg-transparent border-1 form-control form-control-sm text-light" type="number">
                  </td>
                  <td class="text-start d-grid">
                    <button type="button" @click="addClass" class="btn btn-sm btn-primary"><span class="text-light"><i class="bi-plus-circle"></i></span> Add</button>
                  </td>
                </tr>
                </tbody>
              </table>
